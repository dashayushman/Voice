...
class TrainingInstance:
    ...
    def resampleData(self,sr,avg_len,emg=True,imu=True):
        '''
        Method for resampling the all the signals and bringing them to the same sampling rate
        ...
        if self.splitted == True:
            ...
            sample_len_emg_others = int((sr * self.accList[0].size) /self.sr_other )
            ...
            self.emgList = np.array([signal.resample(x, sample_len_emg) for x in self.emgList])
            self.accList = np.array([signal.resample(x, sample_len_emg_others) for x in self.accList])
            self.gyrList = np.array([signal.resample(x, sample_len_emg_others) for x in self.gyrList])
    ...
    def extractFeatures(self, window=True,scaler = None,rms=False,f_mfcc=False,emg=True,imu=True):
        '''
        This method extracts features from the training instance and consolidates into one meature matrix according to the parameters provided
        :param window: (Boolean)                            : To get overlapping windowed features
        :param scaler: (Scaler Object as in scikit-learn)   : Scalar object to scale the features
        :param rms: (Boolean)                               : To extract features from the Root Mean Square of the signals in all dimensions
        :param f_mfcc: (Boolean)                            : To extract MFCC features
        :param emg: (Boolean)                               : To extract features from EMG signals
        :param imu: (Boolean)                               : To extract features from IMU signals
        :return: self
        '''
        #print(self.m_label)
        if self.splitted == True:
    ...
    def consolidateFeatures(self,scaler=None,rms=False,emg=True,imu=True):
        '''
        Method to consolidate the features of all the sensor data in all dimensions to a single feature matrix
        :param scaler: (Scaler Object)      : A scaler object to scale the features
        :param rms: (Boolean)               : Flag for consolidating RMS features
        :param emg: (Boolean)               : Flas to consider features from EMG signals
        :param imu: (Boolean)               : Flag to consider IMU Signals
        :return: consolidated_feature_Matrix (ndarray) : with columns as features and rows as overlapping window frames. If window was false then it just has one row.
        '''
        if self.splitted == True:
            con_emg_feat = None
            con_acc_feat = None
    ...
    def consolidateData(self,avg_len,emg,imu):
        ...
        if self.splitted == True:
            if emg and imu:
                emg_r = np.array([signal.resample(x, avg_len) for x in self.emgList_r])
                acc_r = np.array([signal.resample(x, avg_len) for x in self.accList_r])
                gyr_r = np.array([signal.resample(x, avg_len) for x in self.gyrList_r])
                ori_r = np.array([signal.resample(x, avg_len) for x in self.oriList_r])
                consolidatedDataMatrix = np.concatenate((emg_r,acc_r,gyr_r,ori_r),axis=0)
            elif emg and (not imu):
                consolidatedDataMatrix = self.emgList
            elif (not emg) and imu:
                consolidatedDataMatrix = np.concatenate((self.accList, self.gyrList, self.oriList), axis=0)
            else:
                emg_r = np.array([signal.resample(x, avg_len) for x in self.emgList_r])
                acc_r = np.array([signal.resample(x, avg_len) for x in self.accList_r])
                gyr_r = np.array([signal.resample(x, avg_len) for x in self.gyrList_r])
